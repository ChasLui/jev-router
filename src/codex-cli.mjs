  ...
  if (hasCredentials()) {
    ...
    process.stderr.write(
      "[jev] no routing credentials found - starting Codex without routing\n" +
        ...
        "[jev] CLOUDFLARE_API_TOKEN_JEV and CLOUDFLARE_ACCOUNT_ID, or JEV_PROVIDER=vercel with\n" +
        `[jev] VERCEL_AI_GATEWAY_API_KEY_JEV, in ${join(homedir(), ".jev-router.env")} to enable routing\n`,
    );
  }
